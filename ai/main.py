...
while True:
    ...
    if selected_object is None:
        ...
        for det in detections:
            ...
            if area > max_area:
                max_area = area
                selected_object = det
                print(f"selected_object: ", selected_object)

        if selected_object:
            print(f"Object selected: {selected_object}")
        else:
    ...
    # 선택된 객체 추적
    if selected_object:
        x1, y1, x2, y2 = map(int, selected_object)
        x_center = (x1 + x2) // 2
        y_center = (y1 + y2) // 2
        ...
        if last_center:
            ...
            if movement > movement_threshold:
                print("Object moved. Reinitializing detection.")
                # 객체 초기화 및 재탐지 준비
            ...
            for i in range(detections.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    try:
                        ...
                        with open("output.txt", "w") as f:
                            f.write("1")
    print(f"original_age_index: {original_age_index}")
